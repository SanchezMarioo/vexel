...
export default function ContactForm() {
  ...
  if (status === "success") {
    return (
      <div
        role="status"
        className="route-enter flex h-full min-h-[20rem] flex-col items-start justify-center rounded-[var(--pf-radius-lg)] border border-pf-line bg-pf-surface p-8"
      >
  ...
  return (
    <form
      ...
      noValidate
      className="route-enter rounded-[var(--pf-radius-lg)] border border-pf-line bg-pf-surface p-6 md:p-8"
    >
      {serverError ? (
        <p
          role="alert"
          className="route-enter mb-6 rounded-[var(--pf-radius)] border border-pf-danger bg-pf-surface px-4 py-3 text-sm text-pf-danger"
        >
          {serverError}
        </p>
      ) : null}
